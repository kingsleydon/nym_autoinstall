...
function nym_chmod() {
  ...
  if ls -la /home/nym/ | grep nym-mixnode_linux_x86_64 >/dev/null 2>&1; then
    printf "%b\n\n\n" "${WHITE} --------------------------------------------------------------------------------"
...
function downloader() {
  #set -x
  if [ ! -d /home/nym/.nym/mixnodes ]; then
  ...
  #  then
  if systemctl list-units --state=running | grep nym-mixnode; then
    echo "stopping nym-mixnode.service to update the node ..." && systemctl kill --signal=SIGINT nym-mixnode
...
function upgrade_nym() {
  ...
  sleep 2
  sudo -u nym -H ./nym-mixnode_linux_x86_64 init --id $directory
}
#set -x
